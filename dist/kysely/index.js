...
var getRows = (meta, query, sortTable, executeQuery) => {
  ...
  if (meta.type === "offset") {
    ...
    const orderBy = meta.sort.map((sort2) => `${sortTable}.${sort2.field} ${sort2.dir}`);
...
var getLastPageCursor = async (meta, totalRows, query, sortTable) => {
  ...
  const { sort: sort3 } = meta;
  const sortDir = invertSort(sort3?.dir ?? "asc");
  if (sort3) {
    query = query.orderBy(`${sortTable}.${sort3.field}`, sortDir);
  }
  if (sort3?.field !== meta.idColumn) {
  ...
  query = query.limit(1);
  const row = await query.select(`${sortTable}.${meta.idColumn}`).$if(!!sort3 && sort3.field !== meta.idColumn, (qb) => qb.select(`${sortTable}.${sort3.field}`)).executeTakeFirst();
  if (!row) {
    return null;
  }
  return {
    id: row[meta.idColumn],
    sort: sort3 && row[sort3.field] || undefined
  };
};
