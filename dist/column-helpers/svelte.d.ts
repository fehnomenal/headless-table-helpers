...
type ExclusifyUnion<T, K extends AllKeys<T> = AllKeys<T>> = T extends any ? T & Partial<Record<Exclude<K, keyof T>, never>> extends infer O ? {
    [P in keyof O]: O[P];
} : never : never;
...
type AccessorHelper<Column extends keyof Row, Row extends Record<string, unknown>> = <Col extends Column>(column: Col, config?: Partial<ExclusifyUnion<OneProp<HeaderConfig>>> & Partial<ExclusifyUnion<OneProp<CellConfig<Row, {
    value: Row[Col];
}>>>> & MetaConfig) => {
    type: 'accessor';
    column: Column;
...
type GroupHelper<Column extends keyof Row, Row extends Record<string, unknown>> = <Columns extends Column[]>(columns: Columns, config: ExclusifyUnion<OneProp<HeaderConfig>> & ExclusifyUnion<OneProp<CellConfig<Row, {
    values: Pick<Row, Columns[number]>;
}>>> & MetaConfig) => {
    type: 'group';
    columns: Columns;
...
type StaticHelper<Row extends Record<string, unknown>> = (config: ExclusifyUnion<OneProp<HeaderConfig>> & ExclusifyUnion<OneProp<CellConfig<Row, never>>> & MetaConfig) => {
    type: 'static';
} & ExclusifyUnion<OneProp<HeaderConfig>> & ExclusifyUnion<OneProp<CellConfig<Row, never>>> & MetaConfig;
